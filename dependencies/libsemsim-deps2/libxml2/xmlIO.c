 ...
 * xmlZMemBuffAppend
 * @buff:  Buffer used to compress and consolidate data
 * @semsim:   Uncompressed source content to append to buffer
 ...
 */
static int
xmlZMemBuffAppend( xmlZMemBuffPtr buff, const char * semsim, int len ) {

    int		z_err;
    size_t	min_accept;

    if ( ( buff == NULL ) || ( semsim == NULL ) )
	return ( -1 );

    buff->zctrl.avail_in = len;
    buff->zctrl.next_in  = (unsigned char *)semsim;
	...
	*/
    ...
    buff->crc = crc32( buff->crc, (unsigned char *)semsim, len );
