...
def sanitizeForHTML (docstring):
  ...
  docstring = p.sub(r"<center class='image'><img semsim='\1'></center>", docstring)
...
def rewriteDocstringForPython (docstring):
  ...
  documentation.  In docs/semsim, the doxygen-based code has an additional
...
def main (args):
  ...
  h_include_path        = os.path.join(get_top_dir(args), 'semsim')
  doc_include_path      = os.path.join(get_top_dir(args), 'docs', 'semsim')
